...
function Test() {
    ...
    const onValid = async () => {
        ...
        const response = await client.post("polls/result/", data).then((res) => res.data);
        ...
        {data?.map((question, idx) => {
          return (
            <div className="w-[350px] shadow-xl rounded-xl bg-[#FFFFFF] mb-5">
              ...
              <MbtiSelect positive={question.positive} negative={question.negative} selectValue={(value) => setValue(idx, value)} />
            </div>
          );
        })}
      </div>
